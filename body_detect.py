...
def detect_bodies(source=None, port=None):
    # Memuat Haar Cascade classifier bawaan OpenCV untuk deteksi tubuh penuh
    ...
    if source is not None and os.path.exists(source):
        # =====================================================================
        # MODE 1: Deteksi Tubuh pada File Gambar (Static Image Mode)
        # =====================================================================
        ...
        print(f"Jumlah tubuh terdeteksi: {len(bodies)}")
        
        for i, (x, y, w, h) in enumerate(bodies, 1):
            # Hitung titik tengah tubuh
            center_x = x + (w // 2)
            center_y = y + (h // 2)
            ...
            print(f"Tubuh #{i}: Area ({x}, {y}) s/d ({x+w}, {y+h}) | Titik Tengah: ({center_x}, {center_y})")
        ...
        cv2.imshow('Body Detection (Tekan tombol apa saja untuk keluar)', frame)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        
    else:
        # =====================================================================
        # MODE 2: Deteksi Tubuh Real-time via Webcam + Serial Arduino
        # =====================================================================
        ...
        print("\n=== KENDALI TURRET DETEKSI TUBUH ===")
        print("1. Program dimulai dalam MODE KALIBRASI MANUAL.")
        print("2. Gerakkan JOYSTICK untuk mengarahkan turret tepat ke tengah tubuh target.")
        ...
        while True:
            ...
            # Tentukan tubuh aktif (tubuh pertama yang terdeteksi)
            active_body = None
            if len(bodies) > 0:
                active_body = bodies[0]
                
            # Gambar visualisasi tubuh
            if active_body is not None:
                (x, y, w, h) = active_body
                center_x = x + (w // 2)
                center_y = y + (h // 2)
                
                # Gambar kotak pembatas tubuh target utama
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.circle(frame, (center_x, center_y), 5, (0, 0, 255), -1)
                
                # Tampilkan koordinat tubuh saat ini
                ...
                if is_calibrated:
                    ...
                    # Garis hubung antara titik referensi kalibrasi dan posisi tubuh sekarang
                ...
                # Jika tidak ada tubuh yang terdeteksi
                if is_calibrated:
                    status_text = "AUTO-TRACKING: Tubuh Hilang!"
                    cv2.putText(frame, status_text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                else:
                    status_text = "KALIBRASI: Tubuh tidak terdeteksi"
                    cv2.putText(frame, status_text, (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
            
            # Tampilkan window webcam
            cv2.imshow('Body Detection & Turret Control', frame)
            
            # Input keyboard untuk transisi state
            key = cv2.waitKey(1) & 0xFF
            
            # Tekan 'y' untuk menyelesaikan kalibrasi & kunci posisi tengah tubuh
            if key == ord('y'):
                if active_body is not None:
                    (x, y, w, h) = active_body
                    calib_x = x + (w // 2)
                    ...
                    print("Peringatan: Gagal mengunci kalibrasi karena tubuh tidak terdeteksi.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Program Body Tracking & Turret Controller")
    ...
    detect_bodies(args.image, args.port)
